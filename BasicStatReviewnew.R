...
# 1 Find the height of the normal curve for an x value of -1. (dnorm gives distance between x axis and curve at that x value)
...
# Plotting a T distribution
 ...
 # Finding t distribution probabilities (area under t distribution curve to the left of 2)
 
 pt(q = 2, df = 2)
 
 
 # Finding t distribution values (associated with a given area under the curve)
 
 qt(p = 0.9082, df = 2)
 
 #Hypothesis Testing may be on midterm (through one-sample t-test)
 ...
 boxplot(k) #shows no obvious outliers
 ...
 #also the confidence interval does not contain 100
 
#for a less-than test, change  to "less" 
...
#reject the null hypothesis
 ...
 #var.equal = FALSE says variances are not equal (not that important)
#we fail to reject the null hypothesis because the p-value is too high
