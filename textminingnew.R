...
# Step 1 Locate the text in R Studio and create a tibble
...
#if you want to see more output, indicate how many rows you want to see (in this case, 33 rows)
print(tibble2, n = 33)
...
#find frequencies of words that appear at least four times
tibble2%>%
  count(word, sort =TRUE) %>% 
  filter(n >= 4) 
...
text_tibble <- tibble(line = 75:88, text = text)
...
# Step 3  Find frequencies for each word that appears at least 4 times
tibble2%>%
  count(word, sort =TRUE) %>% 
  filter(n >= 4) 
...
#Create a word cloud for a body of text that you choose

text2 <- readLines(file.choose("CatHatPoem"))
text2
...
#to see rows that have a positive connotation:
#outputs a tibble. to see rows that have a positive connotation , use slice to look at words between (for example) 2000 and 2500, then ...?

mytibble <- get_sentiments("bing")
mytibble %>%
  filter(sentiment == "positive") -> mytibble2

print(mytibble2, n = 50)
...
# words in the nrc wordbank that suggest a sentiment of Joy !
tidy_books%>%
  filter(book == "Emma") %>%
  inner_join(nrc_joy) %>%
  count(word, sort = TRUE) 
...
#another strategy for making word clouds, excluding common words that don't contribute meaningfully
tidy_books %>%
