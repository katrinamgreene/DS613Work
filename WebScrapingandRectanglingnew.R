...
# Let's add another column (day/night designations) !!
...
# example 1 -- extract a table from the internet

wikiurl <- read_html(
  "https://en.wikipedia.org/wiki/List_of_highest-grossing_films")
datatables <- wikiurl%>%
  html_table(., fill = T) #fill = T means get all content

#there are several tables, indicate which one we want a table 4
datatables[[4]] -> dt
dt
...
# Let's find a linear model for number at bats vs home runs (scenario doesn't make sense-- predicting at bats with hits)
lm(ABAB ~ HH, BD2019)

# data was transferred in as tibble by default?
#Let's change the data table to a data frame

BD2019 <- data.frame(baseballdata2019[[1]])
BD2019

# ( the tibble representation is probably better -- more professional, shows first 10? rows only)
...
census_api_key("85a7059c34e275e20ce3ed1c6366391f1893d134")
install = TRUE
...
#reorder is arranging by quantity
vt %>%
...
#improve graph by . . . 
#changing size to 1 so dots are more distinct?
# grouping counties together?
#change bin width?
#use a different kind of graph? -- bar graph?
